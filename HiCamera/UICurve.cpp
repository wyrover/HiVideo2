...
#ifndef CLAMP
#define CLAMP(x, a, b) min((a),max((x),(b)))
#endif // !CLAMP

#ifndef LIMIT
#define LIMIT(x, a, b) {if((x)<(a))(x)=(a);else if((x)>(b))(x)=(b);}
#endif

namespace DuiLib
{
	template<class T> void limit(T & x, T a, T b)
	{
		if (x < a)
			x = a;
		else if (x>b)
...
//  			point.y = clamp((int)point.y, 0, m_nHeight);
			LIMIT(point.x, 0, m_nWidth);
			LIMIT(point.x, 0, m_nHeight);
